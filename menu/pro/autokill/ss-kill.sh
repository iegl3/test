#!/bin/bash
# (C) Copyright 2922-2023 By WildyDev21
# ==================================================================
...
# // Start
echo "$(date) Autokill Shadowsocks MultiLogin for WildyDev21 Script V1.0 Stable";
...
do
    ...
    jumlah_baris_log=$( cat /etc/wildydev21/xray-mini-shadowsocks/access.log | grep -w $hariini | grep -w $waktu | grep -w 'accepted' | grep -w $username | wc -l );
    ...
    if [[ $jumlah_baris_log -gt $SHADOWSOCKS ]]; then
        ...
        echo "SHADOWSOCKS - $(date) - ${username} - Multi Login Detected - Killed at ${date}" >> /etc/wildydev21/autokill.log;
    fi
# // End Function
done
done