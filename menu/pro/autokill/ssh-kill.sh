...
# // Start
echo "$(date) Autokill SSH MultiLogin for WildyDev21 Script V1.0 Stable";
echo "$(date) Coded by wildydev21 ( Version 1.0 Beta )";
echo "$(date) Starting Vmess Autokill Service.";

# // Start Daemon
while true; do
sleep 30
cat /etc/passwd | grep "/home/" | cut -d":" -f1 > /etc/wildydev21/ssh-user.db;
username1=( `cat "/etc/wildydev21/ssh-user.db" `);
i="0";
...
cat $LOG | grep -i dropbear | grep -i "Password auth succeeded" > /etc/wildydev21/ssh-autokill-cache.db;
proc=( `ps aux | grep -i dropbear | awk '{print $2}'`);
for PID in "${proc[@]}"
do
cat /etc/wildydev21/ssh-autokill-cache.db | grep "dropbear\[$PID\]" > /etc/wildydev21/autokill-ssh-cache1.db;
NUM=`cat /etc/wildydev21/autokill-ssh-cache1.db | wc -l`;
USER=`cat /etc/wildydev21/autokill-ssh-cache1.db | awk '{print $10}' | sed 's/'\''//g'`;
IP=`cat /etc/wildydev21/autokill-ssh-cache1.db | awk '{print $12}'`;
if [ $NUM -eq 1 ]; then
i=0;
...
cat $LOG | grep -i sshd | grep -i "Accepted password for" > /etc/wildydev21/ssh-autokill-cache.db;
data=( `ps aux | grep "\[priv\]" | sort -k 72 | awk '{print $2}'`);
for PID in "${data[@]}"
do
cat /etc/wildydev21/ssh-autokill-cache.db | grep "sshd\[$PID\]" > /etc/wildydev21/autokill-ssh-cache1.db;
NUM=`cat /etc/wildydev21/autokill-ssh-cache1.db | wc -l`;
USER=`cat /etc/wildydev21/autokill-ssh-cache1.db | awk '{print $9}'`;
IP=`cat /etc/wildydev21/autokill-ssh-cache1.db | awk '{print $11}'`;
if [ $NUM -eq 1 ]; then
i=0;
for user1 in "${username[@]}"
do
