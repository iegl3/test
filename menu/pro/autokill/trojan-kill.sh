...
# // Start
echo "$(date) Autokill Trojan MultiLogin for WildyDev21 Script V1.0 Stable";
...
do
    ...
    if [[ $jumlah_baris_log -gt $TROJAN ]]; then
        ...
        echo "TROJAN - $(date) - ${username} - Multi Login Detected - Killed at ${date}" >> /etc/wildydev21/autokill.log;
    fi
# // End Function
done
done