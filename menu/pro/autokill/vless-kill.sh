...
# // Start
echo "$(date) Autokill Vless MultiLogin for WildyDev21 Script V1.0 Stable";
...
do
    ...
    cat /etc/wildydev21/xray-mini-nontls/access.log | tail -n30000 > /etc/wildydev21/xray-mini-nontls/cache.log

    # // Configuration user logs
    hariini=`date -d "0 days" +"%Y/%m/%d"`;
    waktu=`date -d "0 days" +"%H:%M"`;
    waktunya=`date -d "0 days" +"%Y/%m/%d %H:%M"`;
    jumlah_baris_log=$( cat /etc/wildydev21/xray-mini-nontls/cache.log | grep -w $hariini | grep -w $waktu | grep -w 'accepted' | grep -w $username | wc -l );
    ...
    if [[ $jumlah_baris_log -gt $VLESS ]]; then
        ...
        echo "VLESS - $(date) - ${username} - Multi Login Detected - Killed at ${date}" >> /etc/wildydev21/autokill.log;
    fi

    # // Creating Cache for access.log
    cat /etc/wildydev21/xray-mini-tls/access.log | tail -n30000 > /etc/wildydev21/xray-mini-tls/cache.log

    # // Configuration user logs
    hariini=`date -d "0 days" +"%Y/%m/%d"`;
    waktu=`date -d "0 days" +"%H:%M"`;
    waktunya=`date -d "0 days" +"%Y/%m/%d %H:%M"`;
    jumlah_baris_log=$( cat /etc/wildydev21/xray-mini-tls/cache.log | grep -w $hariini | grep -w $waktu | grep -w 'accepted' | grep -w $username | wc -l );
