...
# // Start
echo "$(date) Autokill Vmess MultiLogin for WildyDev21 Script V1.0 Stable";
...
do
    ...
    if [[ $jumlah_baris_log -gt $VMESS ]]; then
        ...
        echo "VMESS - $(date) - ${username} - Multi Login Detected - Killed at ${date}" >> /etc/wildydev21/autokill.log;
    fi
